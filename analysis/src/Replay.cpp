...
/*
bool Replay::Process_gainData(const std::string &input_evio, const std::string &output_dat,
                     int max_events, const std::string &daq_config_file)
{
    // build ROC tag → crate index mapping from DAQ config JSON
    std::unordered_map<int, int> roc_to_crate;
    if (!daq_config_file.empty()) {
    ...
    while (ch.Read() == evc::status::success) {
        ...
        for (int ie = 0; ie < ch.GetNEvents(); ++ie) {
            ...
            if ( !(ev->trigger_bits & TBIT_lms) && !(ev->trigger_bits & TBIT_alpha) ) continue;
