...
function registerReportSection(section){
    // section = {id, title, order, generate: async()=>htmlString}
    reportRegistry.push(section);
    reportRegistry.sort((a,b)=>a.order-b.order);
}

// capture geo canvas for a specific tab at fixed resolution
async function captureGeoForTab(tab){
    const prevTab=activeTab, prevW=geoCanvas.width, prevH=geoCanvas.height;
    const prevScale=scale, prevOX=offsetX, prevOY=offsetY;
    // set fixed report resolution
    ...
    // restore
    geoCanvas.width=prevW; geoCanvas.height=prevH;
    canvasW=prevW; canvasH=prevH;
    scale=prevScale; offsetX=prevOX; offsetY=prevOY;
    activeTab=prevTab;
    redrawGeo();
    return url;
}

// render a plotly chart into a temporary off-screen div, capture as image
...
// report-friendly plotly layout (light theme for print)
        ...
        for(const entry of reportRegistry){
            try{
                ...
                sections.push(`<div class="section"><h2>${entry.title}</h2>
                    <p class="no-data">Error generating section: ${err.message}</p></div>`);
            }
        }
...
function buildElogXml(title,logbook,author,tags,bodyHtml,attachments){
    ...
    xml+=`  <Author><username>${escXml(author)}</username></Author>\n`;
    xml+=`  <title>${escXml(title)}</title>\n`;
    xml+=`  <body type="html"><![CDATA[${bodyHtml}]]></body>\n`;
    xml+=`  <Logbooks>\n`;
    for(const lb of logbook.split(',')) xml+=`    <logbook>${escXml(lb.trim())}</logbook>\n`;
    xml+=`  </Logbooks>\n`;
    if(tags&&tags.length){
        xml+=`  <Tags>\n`;
        for(const t of tags) xml+=`    <tag>${escXml(t.trim())}</tag>\n`;
        xml+=`  </Tags>\n`;
    }
    if(attachments&&attachments.length){
        ...
        for(const a of attachments){
            xml+=`    <Attachment>\n`;
            xml+=`      <caption>${escXml(a.caption)}</caption>\n`;
            xml+=`      <filename>${escXml(a.filename)}</filename>\n`;
            xml+=`      <type>${escXml(a.type)}</type>\n`;
            xml+=`      <data encoding="base64">${a.data}</data>\n`;
            xml+=`    </Attachment>\n`;
        }
        xml+=`  </Attachments>\n`;
    }
...
// collected during report generation for elog attachments
...
async function postToElog(){
    ...
    // build simplified body (strip images, keep text/tables)
    const bodyHtml=html.match(/<body[^>]*>([\s\S]*)<\/body>/i);
    let body=bodyHtml?bodyHtml[1]:'';
    // remove base64 images from body (they go as attachments)
    body=body.replace(/<img[^>]*src="data:[^"]*"[^>]*>/gi,'');

    statusEl.textContent='Posting to elog...';
    const xml=buildElogXml(title,logbook,author,tags,body,reportAttachments);

    try{
        const resp=await fetch('/api/elog/post',{method:'POST',headers:{'Content-Type':'application/xml'},body:xml});
...
registerReportSection({id:'dq',title:'Waveform Data (DQ)',order:10,
    generate:async()=>{
        ...
        // geo view
        try{
            const img=await captureGeoForTab('dq');
            const metric=document.getElementById('color-metric').value;
            html+=`<h3>Detector Map (${metric})</h3><img class="geo-img" src="${img}">`;
            addReportAttachment(img,'dq_geo.png','DQ Detector Map');
        }catch(e){}
        // occupancy table
        if(occTotal>0){
            ...
            html+='<table><thead><tr><th style="text-align:left">Module</th><th>Hits</th><th>Occupancy %</th><th>Hits (tcut)</th></tr></thead><tbody>';
            const show=entries.slice(0,50);
            for(const e of show){
                html+=`<tr><td style="text-align:left">${e.name}</td><td>${e.occ}</td>`;
                html+=`<td>${(100*e.occ/occTotal).toFixed(2)}%</td><td>${e.occTcut}</td></tr>`;
            }
            html+='</tbody></table>';
            if(entries.length>50) html+=`<p class="no-data">Showing top 50 of ${entries.length} active modules</p>`;
        }else{
...
registerReportSection({id:'cluster',title:'Clustering',order:20,
    generate:async()=>{
        ...
        if(!hasHist&&!hasClusters) return null;  // skip if no data at all
        ...
        if(hasHist){
            try{
                const img=await plotToImage(async div=>{
                    const x=clHistBins.map((_,i)=>clHistMin+(i+0.5)*clHistStep);
                    const entries=clHistBins.reduce((a,b)=>a+b,0);
                    await Plotly.newPlot(div,[{x,y:clHistBins,type:'bar',
                        marker:{color:'#ff922b',line:{width:0}}}],
                        {...RPL,title:{text:`Cluster Energy (${clHistEvents} evts, ${entries} clusters)`,font:{size:12,color:'#222'}},
                         xaxis:{...RPL.xaxis,title:'Energy (MeV)'},yaxis:{...RPL.yaxis,title:'Counts'},bargap:0.05});
                },800,300);
                html+=`<h3>Cluster Energy Distribution</h3><img class="chart-img" src="${img}">`;
                addReportAttachment(img,'cluster_energy.png','Cluster Energy Histogram');
            }catch(e){}
        }
        // stat histograms
        const statImgs=[];
        if(nclustBins&&nclustBins.some(b=>b>0)){
            try{
                const img=await plotToImage(async div=>{
                    const x=nclustBins.map((_,i)=>nclustMin+i*nclustStep);
                    await Plotly.newPlot(div,[{x,y:nclustBins,type:'bar',
                        marker:{color:'#00b4d8',line:{width:0}}}],
                        {...RPL,title:{text:'Clusters per Event',font:{size:12,color:'#222'}},
                         xaxis:{...RPL.xaxis,title:'# Clusters'},yaxis:{...RPL.yaxis,title:'Counts'},bargap:0.05});
                },400,260);
                statImgs.push(img);
                addReportAttachment(img,'clusters_per_event.png','Clusters per Event');
            }catch(e){}
        }
        if(nblocksBins&&nblocksBins.some(b=>b>0)){
            try{
                const img=await plotToImage(async div=>{
                    const x=nblocksBins.map((_,i)=>nblocksMin+i*nblocksStep);
                    await Plotly.newPlot(div,[{x,y:nblocksBins,type:'bar',
                        marker:{color:'#51cf66',line:{width:0}}}],
                        {...RPL,title:{text:'Blocks per Cluster',font:{size:12,color:'#222'}},
                         xaxis:{...RPL.xaxis,title:'# Blocks'},yaxis:{...RPL.yaxis,title:'Counts'},bargap:0.05});
                },400,260);
                statImgs.push(img);
                addReportAttachment(img,'blocks_per_cluster.png','Blocks per Cluster');
            }catch(e){}
        }
        if(statImgs.length){
        ...
        if(hasClusters){
            ...
            html+='<table><thead><tr><th>#</th><th style="text-align:left">Center</th><th>Energy [MeV]</th>';
            html+='<th>X [mm]</th><th>Y [mm]</th><th>Blocks</th></tr></thead><tbody>';
            clusterData.clusters.forEach((cl,i)=>{
                html+=`<tr><td>${i}</td><td style="text-align:left">${cl.center}</td>`;
                html+=`<td>${cl.energy.toFixed(1)}</td><td>${cl.x.toFixed(1)}</td>`;
                html+=`<td>${cl.y.toFixed(1)}</td><td>${cl.nblocks}</td></tr>`;
            });
            html+='</tbody></table>';
        }
        html+='</div>';
...
registerReportSection({id:'lms',title:'Gain Monitoring (LMS)',order:30,
    generate:async()=>{
        ...
        // geo view
        try{
            const img=await captureGeoForTab('lms');
            html+=`<h3>LMS Status Map</h3><img class="geo-img" src="${img}">`;
            addReportAttachment(img,'lms_geo.png','LMS Status Map');
        }catch(e){}
        // summary
        entries.sort((a,b)=>{
            if(a.warn!==b.warn) return a.warn?-1:1;
        ...
        html+='<table><thead><tr><th style="text-align:left">Module</th><th>Mean</th><th>RMS</th>';
        html+='<th>RMS/Mean %</th><th>Count</th><th>Status</th></tr></thead><tbody>';
        for(const e of entries){
            const rmsFrac=e.mean>0?(e.rms/e.mean*100).toFixed(1):'--';
            const status=e.warn?'<span class="warn">WARN</span>':'<span class="ok">OK</span>';
            html+=`<tr><td style="text-align:left">${e.name}</td>`;
            html+=`<td>${e.mean.toFixed(1)}</td><td>${e.rms.toFixed(2)}</td>`;
            html+=`<td>${rmsFrac}%</td><td>${e.count}</td><td>${status}</td></tr>`;
        }
...
function init(){
    ...
    document.getElementById('btn-report-pdf').onclick=()=>{reportMenu.classList.remove('open');downloadReportPdf();};
    document.getElementById('btn-report-elog').onclick=()=>{reportMenu.classList.remove('open');showElogDialog();};
    ...
    fetch('/api/config').then(r=>r.json()).then(data=>{
        ...
        if(data.elog){
            ...
            // hide elog button if no URL configured
            if(!data.elog.url){
